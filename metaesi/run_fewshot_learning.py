...
def Kshot_learning(model_name, model_path, dataset, esi_count, Feature, coeffs, epochs, save_path, gpu):
    ...
    for few_e3 in Dataset.esi_count['E3'].values:

        kshot_esi = Dataset.sample_kshot_ESIs(few_e3)
        MetaESI_e3 = train_MetaESI(copy.deepcopy(model), kshot_esi, Feature, epochs, gpu, mini_batch = -1)
        torch.save(MetaESI_e3.state_dict(), save_path + few_e3 + ".pth")
