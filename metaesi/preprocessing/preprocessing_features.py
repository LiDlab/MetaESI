...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '../..', 'gard'))
...
def load_features_esm2(AC, data_path):
    embedding_file = torch.load(data_path + AC + ".pt")
    embedding = embedding_file['representations'][33].numpy()

    return embedding

save_path = "../../data/human/features/"
feature_path = "../../data/human/esm2/"

gard_features = h5py.File(save_path + "GARD_features.hdf5", "r")
sequences_dict = read_fasta(save_path + "MetaESI_seq.fasta")
METAESI_features_e3 = h5py.File(save_path + "METAESI_features_e3.hdf5", "w")
METAESI_features_sub = h5py.File(save_path + "METAESI_features_sub.hdf5", "w")

gard_thr = 1.5

protein = list(gard_features.keys())

for i in tqdm(protein):
