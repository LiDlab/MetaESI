...
def read_cif_and_calculate_distances(cif_file_path):
    """
    Parse CIF file and calculate C-alpha distance matrix between two chains.

    Args:
        cif_file_path (str): Path to input CIF file

    Returns:
        np.ndarray: Distance matrix between C-alpha atoms of chain A and B
    """
    ...
    for chain in chains:
        if chain.get_id() == "A":
            x0_residues = [residue['CA'].get_coord() for residue in chain if residue.has_id('CA')]
        elif chain.get_id() == "B":
            x1_residues = [residue['CA'].get_coord() for residue in chain if residue.has_id('CA')]

    # Calculate distance matrix
    distance_matrix = np.zeros((len(x0_residues), len(x1_residues)))

    for i, x0_residue in enumerate(x0_residues):
        for j, x1_residue in enumerate(x1_residues):
            distance_matrix[i, j] = np.linalg.norm(x0_residue - x1_residue)
...
def generate_interface_EMD(val):
    ...
    for e3 in tqdm(all_e3):
        ...
        for idx in range(len(dataset_val_e3)):
            ...
            file_name = "{}_{}_AF3.cif".format(x0, x1)

            if file_name in os.listdir("../../results/alphafold3"):

                # Load interface predictions
                cif_file_path = os.path.join("../../results/alphafold3", file_name)
                distance_matrix = read_cif_and_calculate_distances(cif_file_path)
...
if __name__=="__main__":
    ...
    boxplot.to_csv(save_path + "Interface_EMD_by_AF3.csv", index=False)
