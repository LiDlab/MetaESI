...
from eval_EMD_PeSTo import extract_alphaC_bfactors, construct_interaction_map
...
def generate_interface_EMD(val):
    ...
    for e3 in tqdm(all_e3):
        ...
        for idx in range(len(dataset_val_e3)):
            ...
            if (f"annotated_AF-{x0}-F1-model_v2.pdb" in os.listdir("../../results/ScanNet")) & (f"annotated_AF-{x1}-F1-model_v2.pdb" in os.listdir("../../results/ScanNet")):

                # Interface prediction and processing
                e3_file_path = os.path.join("../../results/ScanNet", f"annotated_AF-{x0}-F1-model_v2.pdb")
                sub_file_path = os.path.join("../../results/ScanNet", f"annotated_AF-{x1}-F1-model_v2.pdb")
...
if __name__=="__main__":
    ...
    boxplot.to_csv(save_path + "Interface_EMD_by_ScanNet.csv", index=False)
