...
def generate_interface_EMD(val, Dataset, Feature, gpu, num_rand):
    ...
    df = pd.DataFrame(columns=('MetaESI Score', 'MetaESI Interface Residues (E3)', 'MetaESI Interface Residues (SUB)', 'True Positive Residues (E3)', 'True Positive Residues (SUB)'
    ...
    for e3 in tqdm(all_e3):
        ...
        for idx, (e3_E, e3_A, sub_E_list, label) in enumerate(Feature):
            ...
            if (float(p_hat) < 0.75):
                ...
                df.loc[t] = [str(p_hat.item()), str(pred1), str(pred2), str(list(np.intersect1d(pred1, val1))),
